...
export function CommandPalette({
  ...
  let panelTransition: Transition = { duration: 0.12, ease: EASE_OUT };
  if (reduce) {
    panelTransition = { duration: 0.1 };
  } else if (open) {
    panelTransition = PANEL_SPRING;
  }
            ...
            {filtered.length === 0 ? (
              ...
              grouped.map(([group, list]) => (
                <div className="mb-1 last:mb-0" key={group}>
                  ...
                  {/* biome-ignore lint/complexity/noExcessiveCognitiveComplexity: per-item row renders active state, icon slot, and highlight in one place */}
                  {list.map((it) => {
                    const idx = cursor++;
